...
{
    public class TIMS_ContractorController : BaseController
    {
        private TIMS_Contractor _routeFilter;
        public TIMS_Contractor RouteFilter
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(ui_route_filter))
                {
                    try
                    {
                        ...
                        var filter = JsonConvert.DeserializeObject<TIMS_ContractorViewModel>(ui_route_filter).ToModel();
        ...
        public List<TIMS_ContractorViewModel> GetList()
        {
            db.Configuration.ProxyCreationEnabled = false;
            var data = db.TIMS_Contractor.AsQueryable();
            ...
            var results = data.ToList().Select(x => new TIMS_ContractorViewModel(x, true)).ToList();

            return results;
        }

        public TIMS_Contractor Get(Guid id)
        {
            db.Configuration.ProxyCreationEnabled = false;
            return db.TIMS_Contractor.FirstOrDefault(x=> x.ID == id);
        }
        ...
        public ActionResult Details(Guid id)
        {
            ...
            var vm = new TIMS_ContractorViewModel(m, true);
        ...
        {
            var vm = RouteFilter != null ? new TIMS_ContractorViewModel(RouteFilter) : new TIMS_ContractorViewModel() {  };
        ...
        public ActionResult Edit(Guid id)
        {
            ...
            var vm = new TIMS_ContractorViewModel(m);
            ViewBag.Lookups = GetLookups();
        ...
        public ActionResult Create(TIMS_ContractorViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
                m.ID = Guid.NewGuid(); 
                db.TIMS_Contractor.Add(m);
                db.SaveChanges();
                return List(m.ID);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Update(TIMS_ContractorViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(TIMS_ContractorViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var em = Get(vm.ID);
