...
namespace WorkflowWeb.Controllers
{
    public class TIMS_UserRoleController : BaseController
    {
        public List<TIMS_UserRoleViewModel> GetList()
        {
            ...
            var data = db.TIMS_UserRole.Include(x => x.TIMS_User)
        ...
        public TIMS_UserRole Get(Guid id)
        {
            ...
            return db.TIMS_UserRole.Include(x => x.TIMS_User)
				.Include(x => x.TIMS_Project)
				.Include(x => x.TIMS_ProjectPackage)
				.Include(x => x.TIMS_Role).FirstOrDefault(x=> x.ID == id);
        }
        ...
        public Dictionary<string, object> GetLookups()
        {
            return new Dictionary<string, object> {
				...
				{"ProjectPackageID", db.TIMS_ProjectPackage.Select(x => new  SelectListItem { Value = x.ID.ToString(), Text = x.Name.ToString() }) },
				{"RoleID", db.TIMS_Role.Select(x => new  SelectListItem { Value = x.ID.ToString(), Text = x.Name.ToString() }) },
        ...
        public ActionResult Edit(Guid id)
        {
            ...
            var vm = new TIMS_UserRoleViewModel(m);
            ViewBag.Lookups = GetLookups();
