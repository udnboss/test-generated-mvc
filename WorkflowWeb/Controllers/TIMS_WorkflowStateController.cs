...
{
    public class TIMS_WorkflowStateController : BaseController
    {
        private TIMS_WorkflowState _routeFilter;
        public TIMS_WorkflowState RouteFilter
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(ui_route_filter))
                {
                    try
                    {
                        ...
                        var filter = JsonConvert.DeserializeObject<TIMS_WorkflowStateViewModel>(ui_route_filter).ToModel();
        ...
        public List<TIMS_WorkflowStateViewModel> GetList()
        {
            db.Configuration.ProxyCreationEnabled = false;
            var data = db.TIMS_WorkflowState.AsQueryable();
            ...
            var results = data.ToList().Select(x => new TIMS_WorkflowStateViewModel(x, true)).ToList();

            return results;
        }

        public TIMS_WorkflowState Get(String id)
        {
            db.Configuration.ProxyCreationEnabled = false;
            return db.TIMS_WorkflowState.FirstOrDefault(x=> x.ID == id);
        }
        ...
        public ActionResult Details(String id)
        {
            ...
            var vm = new TIMS_WorkflowStateViewModel(m, true);
        ...
        {
            var vm = RouteFilter != null ? new TIMS_WorkflowStateViewModel(RouteFilter) : new TIMS_WorkflowStateViewModel() {  };
        ...
        public ActionResult Edit(String id)
        {
            ...
            var vm = new TIMS_WorkflowStateViewModel(m);
            ViewBag.Lookups = GetLookups();
        ...
        public ActionResult Create(TIMS_WorkflowStateViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
                
                db.TIMS_WorkflowState.Add(m);
                db.SaveChanges();
                return List(m.ID);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Update(TIMS_WorkflowStateViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(TIMS_WorkflowStateViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var em = Get(vm.ID);
                ...
                db.TIMS_WorkflowState.Remove(em);
                db.SaveChanges();
