...
namespace WorkflowWeb.Controllers
{
    public class TIMS_ProjectInterfacePointFieldEntryController : BaseController
    {
        ...
        public List<TIMS_ProjectInterfacePointFieldEntryViewModel> GetList()
        {
            ...
            var data = db.TIMS_ProjectInterfacePointFieldEntry.Include(x => x.TIMS_ProjectInterfacePointWorkflow)
        ...
        public TIMS_ProjectInterfacePointFieldEntry Get(Guid id)
        {
            ...
            return db.TIMS_ProjectInterfacePointFieldEntry.Include(x => x.TIMS_ProjectInterfacePointWorkflow)
				.Include(x => x.TIMS_ProjectDisciplineInterfaceTypeField).FirstOrDefault(x=> x.ID == id);
        }
        ...
        public ActionResult Details(Guid id)
        {
            ...
            var vm = new TIMS_ProjectInterfacePointFieldEntryViewModel(m, true);
        ...
        public ActionResult Edit(Guid id)
        {
            ...
            var vm = new TIMS_ProjectInterfacePointFieldEntryViewModel(m);
            ViewBag.Lookups = GetLookups();
        ...
        public ActionResult Create(TIMS_ProjectInterfacePointFieldEntryViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
                m.ID = Guid.NewGuid(); 
                db.TIMS_ProjectInterfacePointFieldEntry.Add(m);
                db.SaveChanges();
                return List(m.ID);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Update(TIMS_ProjectInterfacePointFieldEntryViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(TIMS_ProjectInterfacePointFieldEntryViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var em = Get(vm.ID);
                ...
                db.TIMS_ProjectInterfacePointFieldEntry.Remove(em);
                db.SaveChanges();
