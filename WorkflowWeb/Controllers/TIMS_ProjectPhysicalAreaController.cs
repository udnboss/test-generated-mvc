...
{
    public class TIMS_ProjectPhysicalAreaController : BaseController
    {
        private TIMS_ProjectPhysicalArea _routeFilter;
        public TIMS_ProjectPhysicalArea RouteFilter
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(ui_route_filter))
                {
                    try
                    {
                        ...
                        var filter = JsonConvert.DeserializeObject<TIMS_ProjectPhysicalAreaViewModel>(ui_route_filter).ToModel();
        ...
        public List<TIMS_ProjectPhysicalAreaViewModel> GetList()
        {
            db.Configuration.ProxyCreationEnabled = false;
            var data = db.TIMS_ProjectPhysicalArea.Include(x => x.TIMS_Project).AsQueryable();
            ...
            var results = data.ToList().Select(x => new TIMS_ProjectPhysicalAreaViewModel(x, true)).ToList();

            return results;
        }

        public TIMS_ProjectPhysicalArea Get(Guid id)
        {
            db.Configuration.ProxyCreationEnabled = false;
            return db.TIMS_ProjectPhysicalArea.Include(x => x.TIMS_Project).FirstOrDefault(x=> x.ID == id);
        }
        ...
        public ActionResult Edit(Guid id)
        {
            ...
            var vm = new TIMS_ProjectPhysicalAreaViewModel(m);
            ViewBag.Lookups = GetLookups();
        ...
        public ActionResult Create(TIMS_ProjectPhysicalAreaViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
                m.ID = Guid.NewGuid(); 
                db.TIMS_ProjectPhysicalArea.Add(m);
                db.SaveChanges();
                return List(m.ID);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Update(TIMS_ProjectPhysicalAreaViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var m = vm.ToModel();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(TIMS_ProjectPhysicalAreaViewModel vm)
        {
            if (ModelState.IsValid)
            {
                var em = Get(vm.ID);
                ...
                db.TIMS_ProjectPhysicalArea.Remove(em);
                db.SaveChanges();
