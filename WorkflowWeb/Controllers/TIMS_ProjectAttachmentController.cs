...
{
    public partial class TIMS_ProjectAttachmentController : BaseController<TIMS_ProjectAttachment, TIMS_ProjectAttachmentBusiness, TIMS_ProjectAttachmentViewModel>
    {
        public TIMS_ProjectAttachmentController()
        {
            business = new TIMS_ProjectAttachmentBusiness(db, user);
        ...
        public Dictionary<string, object> DefaultGetLookups()
        {
            ...
            return new Dictionary<string, object> {
                {"ProjectActionItemWorkflowID", db.TIMS_ProjectActionItemWorkflow.Where(x => routeFilter.ProjectActionItemWorkflowID == null || x.ID == routeFilter.ProjectActionItemWorkflowID).Select(x => new  SelectListItem { Value = x.ID.ToString(), Text = x.ID.ToString() }) },
				{"ProjectInterfaceAgreementWorkflowID", db.TIMS_ProjectInterfaceAgreementWorkflow.Where(x => routeFilter.ProjectInterfaceAgreementWorkflowID == null || x.ID == routeFilter.ProjectInterfaceAgreementWorkflowID).Select(x => new  SelectListItem { Value = x.ID.ToString(), Text = x.ID.ToString() }) },
				{"ProjectInterfacePointWorkflowID", db.TIMS_ProjectInterfacePointWorkflow.Where(x => routeFilter.ProjectInterfacePointWorkflowID == null || x.ID == routeFilter.ProjectInterfacePointWorkflowID).Select(x => new  SelectListItem { Value = x.ID.ToString(), Text = x.ID.ToString() }) },
				{"PackageID", db.TIMS_ProjectPackage.Where(x => routeFilter.PackageID == null || x.ID == routeFilter.PackageID).Select(x => new  SelectListItem { Value = x.ID.ToString(), Text = x.Name.ToString() }) },
        ...
        public ActionResult List(Guid? id = null, string ui_list_view = null, bool json = false)
        {
            ...
            {
                var data = results.Data.Select(x => new TIMS_ProjectAttachmentViewModel(x, true)).ToList();
        ...
        public ActionResult Details(Guid id, bool partial = true, bool json = false)
        {
            ...
            else
            {
                ...
                {
                    var m = r.Data;
                    var vm = new TIMS_ProjectAttachmentViewModel(m, true);
        ...
        public ActionResult Create(TIMS_ProjectAttachmentViewModel vm, bool json = false)
        {
            string message;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Update(TIMS_ProjectAttachmentViewModel vm, bool json = false)
        {
            string message;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(TIMS_ProjectAttachmentViewModel vm, bool json = false)
        {
            string message;
